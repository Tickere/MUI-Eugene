// MachinePunnettPanelFramed.swift
import SwiftUI

public struct MachinePunnettPanelFramed: View {
    public let p1A: String, p1B: String, p1C: String
    public let p2A: String, p2B: String, p2C: String

    public init(
        p1A: String, p1B: String, p1C: String,
        p2A: String, p2B: String, p2C: String
    ) {
        self.p1A = p1A; self.p1B = p1B; self.p1C = p1C
        self.p2A = p2A; self.p2B = p2B; self.p2C = p2C
    }

    @State private var tab: Trait = .eyes

    public var body: some View {
        VStack(spacing: 16) {
            Picker("", selection: $tab) {
                Text("Eyes").tag(Trait.eyes)
                Text("Horns").tag(Trait.horns)
                Text("Colour").tag(Trait.colour)
            }
            .pickerStyle(.segmented)

            switch tab {
            case .eyes:
                PunnettSquareView(externalA: p1A, externalB: p2A, traitName: "Eyes")
            case .horns:
                PunnettSquareView(externalA: p1B, externalB: p2B, traitName: "Horns")
            case .colour:
                PunnettSquareView(externalA: p1C, externalB: p2C, traitName: "Colour")
            }
        }
        .padding(.horizontal, 24)
        .padding(.vertical, 20)
        .frame(width: 640, height: 560)        // tall frame to avoid clipping
        .background(
    ...
    enum Trait { case eyes, horns, colour }
}

// MARK: - Punnett Square (auto-filled, read-only)
private struct PunnettSquareView: View {
    ...
    init(externalA: String, externalB: String, traitName: String) {
        self.externalA = externalA
        self.externalB = externalB
        self.traitName = traitName
        _parentA = State(initialValue: externalA)
        _parentB = State(initialValue: externalB)
    }
    ...
    var body: some View {
        ...
        .onChange(of: externalA) { new in parentA = new }
        .onChange(of: externalB) { new in parentB = new }
    }
}
...
private struct PunnettGridView: View {
    let allelesA: [String], allelesB: [String], grid: [[String]]
    private let boxCorner: CGFloat = 16, boxStroke: CGFloat = 2, boxHeight: CGFloat = 160, leftGutter: CGFloat = 32
    ...
    private struct Cell: View {
        let text: String
        var body: some View { Text(text).monospaced().frame(maxWidth: .infinity, maxHeight: .infinity) }
    }
}

// MARK: - Logic
private func twoChars(_ s: String) -> [String] {
    let chars = s.trimmingCharacters(in: .whitespacesAndNewlines).map { String($0) }
...
private func genotypeSummary(_ grid: [[String]]) -> [(String, Int, Double)] {
    ...
    var counts: [String: Int] = [:]; for g in flat { counts[g, default: 0] += 1 }
    let total = Double(flat.count)
    return counts.sorted { $0.key < $1.key }.map { pair in (pair.key, pair.value, Double(pair.value) / total) }
}
...
private func formatPct1(_ p: Double) -> String { String(format: "%.1f%%", p * 100.0) }
